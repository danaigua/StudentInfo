...
<script type="text/javascript">
	function loadClassInfo(){
		...
		$.post("classGetClassByGradeId",{s_gradeId:s_gradeId},
				function(result){
					$("#s_classId").val("-1");
	...
	function studentDelete(studentId){
		if(confirm("确定要删除这条学生信息吗?")){
			$.post("studnetDelete",{studentId:studnetId},
					function(result){
						var result = eval('('+result+')');
						if(result.error){
							alert(result.error);
		...
		<form action="${pageContext.request.contextPath}/studentList" method="post">
			<table align="center">
				<tr>
					...
					<td><input type = "text" id = "s_stuNum" name = "s_student.stuNum" value = "${s_student.stuNum}"/></td>
					<td width="30px">&nbsp;</td>
				...
				<tr>
					<td colspan="2"><button class="btn btn-primary" type="submit">查询</button></td>
					<td>&nbsp;</td>
					<td colspan="2"><button class="btn btn-primary" type="submit" onclick="resetValue()">重置</button></td>
				</tr>
			</table>
		</form>
		<button class="btn btn-mini btn-primary" type="button" style="float: right;margin-bottom: 5px;" onclick="javascript:window.location='studentPreSave'">添加学生</button>
	</div>
		...
		<table class="table table-bordered table-hover">
			...
			<c:forEach var="student" items="${studentList}" varStatus="status">
				<tr>
					...
					<td>
					<button class="btn btn-mini btn-info" type="button" onclick="javascript:window.location='studentShow?studentId=${student.studentId}'">查看详细信息</button>&nbsp;&nbsp;
					<button class="btn btn-mini btn-info" type="button" onclick="javascript:window.location='studentPreSave?studentId=${student.studentId}'">修改</button>&nbsp;&nbsp;
					<button class="btn btn-mini btn-danger" type="button" onclick="studentDelete(${studentId.studentId})">删除</button></td>
				</tr>
			</c:forEach>
		</table>
	</div>
