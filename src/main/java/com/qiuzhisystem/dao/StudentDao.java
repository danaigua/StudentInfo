...
import com.qiuzhisystem.utils.UUIDUtil;
...
public class StudentDao {
	...
	public List<Student> studentList(Connection con, Student s_student, PageBean pageBean)throws Exception{
		...
		if(StringUtil.isNotEmpty(s_student.getStuNum())) {
	...
	public int studentCount(Connection con, Student s_student) throws Exception{
		StringBuffer sb = new StringBuffer("select count(*)total from t_student t1, t_class t2, t_grade t3 where t1.classId = t2.classId and t2.gradeId = t3.gradeId");
		if(StringUtil.isNotEmpty(s_student.getStuNum())) {
			sb.append("  and t1.stuNum like '%" + s_student.getStuNum() + "%'");
		}
		if(StringUtil.isNotEmpty(s_student.getStuName())) {
	...
	public int studentUpdate(Connection con,Student student)throws Exception{
		String sql="update t_student set stuNum=?,stuName=?,stuSex=?,stuBirthday=?,stuRxsj=?,stuNation=?,stuZzmm=?,classId=?,stuDesc=?,stuPic=? where studentId=?";
		PreparedStatement pstmt=con.prepareStatement(sql);
